...
const createTeam = (newTeam, selectedPlayers) => new Promise((resolve, reject) => {
  const teamToPost = {
    ...newTeam,
    playerIds: selectedPlayers.map((player) => player.value),
  };
  axios.post('https://localhost:44317/api/teams', teamToPost)
    .then((result) => {
